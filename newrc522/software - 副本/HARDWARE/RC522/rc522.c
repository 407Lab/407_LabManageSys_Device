...
u8 First(char *num)
{
	...
	if(Card_flag==1)	//如果检测到IC卡
  {
	  ...
	  LCD_ShowString(30,130,200,16,16,"111");
		LCD_ShowString(100,130,200,16,16,"kahao:");
...
u8 Second(char *num)
{
  ...
  if(Card_flag==1)
	{		
		POINT_COLOR=BLUE;
		LCD_ShowString(30,170,180,16,16,"222");
		LCD_ShowString(100,170,180,16,16,"kahao:");
...
void Compare_ID(void)
{
  ...
  char redata2[9];
	...
	if(state==1)
	{ 
		LCD_ShowString(56,250,200,24,24,"OK");
		delay_ms(1800);
		
	}
	else 
	{
		LCD_ShowString(36,250,200,24,24,"please try again");
	  delay_ms(1800);
	}
